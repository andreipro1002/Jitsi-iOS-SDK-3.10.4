...
- (void)audioSessionDidChangeRoute:(RTCAudioSession *)session
    ...
    DDLogInfo(@"[AudioMode] Route changed with previous: %@ --- current: %@", previousRoute.debugDescription, session.currentRoute.outputs.firstObject.debugDescription);
    
    // Update JS about the changes.
    [self notifyDevicesChanged];
    
    if (self->activeMode == kAudioModeVideoCall && self->shouldForceSpeakerForVideo == YES) {
        DDLogInfo(@"[AudioMode] Override to speaker");
